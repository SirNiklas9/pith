...
VS Code runs pith through **Tasks** — the built-in task runner that can prompt for free text, which means `edit` and `generate` work properly here (unlike JetBrains External Tools).
...
4. pith runs, the file is rewritten, VS Code reloads it
